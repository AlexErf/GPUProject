...
bool GCNScheduleDAGMILive::checkNode(SUnit* node, const std::map<int, SUnit*>& scheduleSoFar, const std::vector<SUnit*>& currentScheduledInstructions, unsigned targetLength, unsigned targetAPRP, 
  ...
  std::transform(currentScheduledInstructions.begin(), currentScheduledInstructions.end(), [](const SUnit* s){return s->getInstr();});
  std::sort(history.begin(), history.end());
  static std::unordered_map<std::vector<MachineInstr*>, unsigned> historyMap;
