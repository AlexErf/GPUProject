...
// lid: 256 1 1
// Original:
// X_T1014[n, x0, x1, co : _T1440, _T1441, _T1442, _T1443] = +(X_T1013[n, k0 + x0, k1 + x1, ci] * X_I_389[k0, k1, ci, co])
// With Index Variables Made Integral:
// X_T1014[n, x0, x1, co : _T1440, _T1441, _T1442, _T1443] = +(X_T1013[n, k0 + x0, k1 + x1, ci] * X_I_389[k0, k1, ci, co]), 500000000 + ci < 1000000000, 500000000 + co < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise op: [[pid(Sub)]] X_T1015 = sub(X_T1014, X_I_388)
// Elementwise op: [[pid(Mul)]] X_T1016 = mul(X_T1015, X_I_387)
...
// lwork = 256, 1, 1
// gwork = 1024, 4, 1
__kernel void kernel_c68_sdk_345(__global float* restrict  X_T1016, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_388, __global const float* restrict  X_I_387)
{
  ...
  if (x1_cond)
  {
    for (int x0_lid = 0; x0_lid < 7; x0_lid += 1)
    {
      ...
      float LX_T1015 = (LX_T1014 - LX_I_388);
      float LX_T1016 = (LX_T1015 * LX_I_387);
      X_T1016[gout_idx] = LX_T1016;
    }
  }
}
