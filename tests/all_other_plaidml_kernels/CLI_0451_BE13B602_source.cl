...
// lid: 256 1 1
// Original:
// X_T1586[x0, y1 : _T2285, _T2286] = +(X_T1585[x0, z] * X_I_1[z, y1])
// With Index Variables Made Integral:
// X_T1586[x0, y1 : _T2285, _T2286] = +(X_T1585[x0, z] * X_I_1[z, y1]), 500000000 + x0 < 1000000000, 500000000 + y1 < 1000000000, 500000000 + z < 1000000000
// Constraints:{ 0 <= x0 < 1, 0 <= x0 < 1, 0 <= y1 < 1000, 0 <= y1 < 1000, 0 <= z < 1024, 0 <= z < 1024, 0 <= 500000000 + x0 < 1000000000, 0 <= 500000000 + y1 < 1000000000, 0 <= 500000000 + z < 1000000000 }
// Merged Parallel Constraints:{ 0 <= x0 < 1, 0 <= y1 < 1000, 0 <= z < 1024 }
// Defracted:
// X_T1586[x0, y1 : _T2285, _T2286] = +(X_T1585[x0, z] * X_I_1[z, y1]), 500000000 + x0 < 1000000000, 500000000 + y1 < 1000000000, 500000000 + z < 1000000000
// Flattened:
//              Range   X_T1586   X_T1585     X_I_1  
//       y1      1000         1         0         1  
//        z      1024         0         1      1000  
//      off                   0         0         0  
//      vec                   1         1         1  
...
// Elementwise op: [[pid(Add)]] X_T1587 = add(X_T1586, X_I_0)
// Tile size: { 64, 64 }
...
// lwork = 256, 1, 1
// gwork = 4096, 1, 1
__kernel void kernel_c68_sdk_549(__global float* restrict  X_T1587, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_0)
{
  ...
  for (int z_gid = 0; z_gid < 1024; z_gid += 64)
  {
    {
      int z_tid = (tid % 64);
      if ((tid < 64))
      {
        ...
        in1_shared[z_tid] = in1[clamp((int)gidx, (int)0, (int)1023)];
      }
    }
    {
