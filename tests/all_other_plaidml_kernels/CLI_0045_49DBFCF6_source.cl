...
// lid: 256 1 1
// Original:
// X_T102[n, x0, x1, co : _T47, _T48, _T49, _T50] = +(X_I_179[n, k0 + 2*x0, k1 + 2*x1, ci] * X_I_180[k0, k1, ci, co])
// With Index Variables Made Integral:
// X_T102[n, x0, x1, co : _T47, _T48, _T49, _T50] = +(X_I_179[n, k0 + 2*x0, k1 + 2*x1, ci] * X_I_180[k0, k1, ci, co]), 500000000 + ci < 1000000000, 500000000 + co < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_I_178 shape: fp32(32):(1):128 bytes
// Elementwise input X_I_177 shape: fp32(32):(1):128 bytes
// Elementwise op: [[pid(Sub)]] X_T103 = sub(X_T102, X_I_178)
// Elementwise op: [[pid(Mul)]] X_T104 = mul(X_T103, X_I_177)
// Tile size: { 3, 32, 3, 3, 16, 8 }
// Contraction output var shape: fp32(1, 149, 149, 32):(710432, 4768, 32, 1):2775.12 KiB
// Computed true ops: 76726656
...
// lwork = 256, 1, 1
// gwork = 2560, 19, 1
__kernel void kernel_c28_sdk_34(__global float* restrict  X_T104, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_178, __global const float* restrict  X_I_177)
{
  ...
  for (int x1_lid = 0; x1_lid < 2; x1_lid += 1)
  {
    ...
    if (x1_cond)
    {
      ...
      for (int x0_lid = 0; x0_lid < 8; x0_lid += 1)
      {
        ...
        {
          int x0 = ((2 * x0_lid) + x0_tid);
          float LX_T102 = agg[(x1_lid + (x0_lid * 2))];
          int gout_idx = ((co_tid + (4768 * (x0_gid + x0))) + (32 * (x1_gid + x1)));
          float LX_I_178 = X_I_178[co_tid];
          float LX_I_177 = X_I_177[co_tid];
          float LX_T103 = (LX_T102 - LX_I_178);
          float LX_T104 = (LX_T103 * LX_I_177);
