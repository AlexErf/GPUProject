...
// Elementwise op: [[pid(Add, Switch)]] X_T49 = add(X_T48, X_I_47)
...
// lwork = 256, 1, 1
// gwork = 9472, 37, 1
__kernel void kernel_c56_sdk_5(__global float* restrict  X_T51, __global const float* restrict  X_T43, __global const float* restrict  X_T47, __global const float* restrict  X_I_47)
{
  ...
  if (i3_cond)
  {
    for (int i2_lid = 0; i2_lid < 2; i2_lid += 1)
    {
      ...
      if (i2_cond)
      {
        ...
        float LX_I_47 = X_I_47[i4_tid];
        float LX_T48 = (LX_T43 / LX_T47);
        float LX_T49 = (LX_T48 + LX_I_47);
