...
// lid: 256 1 1
// Original:
// X_T3359[n, x0, x1, c : _T5361, _T5362, _T5363, _T5364] = +(X_T3025[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3
// With Index Variables Made Integral:
// X_T3359[n, x0, x1, c : _T5361, _T5362, _T5363, _T5364] = +(X_T3025[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise op: [[pid(normal_A_block_14)]] X_T3360 = div(X_T3359, X_T2979)
// Elementwise op: [[pid(Add)]] X_T3361 = add(X_T3360, X_T3360)
...
// lwork = 256, 1, 1
// gwork = 768, 11, 1
__kernel void kernel_c42_sdk_1303(__global float* restrict  X_T3361, __global const float* restrict  in1, __global const float* restrict  X_T2979)
{
  in1 = (in1 + -8064);
    ...
    if (c_cond)
    {
      ...
      for (int x0_lid = 0; x0_lid < 2; x0_lid += 1)
      {
        ...
        if (x0_cond)
        {
          ...
          float LX_T3359 = agg[(c_lid + (x0_lid * 8))];
          int gout_idx = (((c_gid + c) + (7392 * x0)) + (672 * x1_gid));
          if (((gout_idx >= 0) && (gout_idx < 81312)))
          {
            float LX_T2979 = X_T2979[gout_idx];
            float LX_T3360 = (LX_T3359 / LX_T2979);
            float LX_T3361 = (LX_T3360 + LX_T3360);
            X_T3361[gout_idx] = LX_T3361;
          }
        }
      }
    }
