...
// lid: 256 1 1
// Original:
// X_T2189[n, x0, x1, c : _T3471, _T3472, _T3473, _T3474] = +(X_T2188[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 3, k1 < 3
// With Index Variables Made Integral:
// X_T2189[n, x0, x1, c : _T3471, _T3472, _T3473, _T3474] = +(X_T2188[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 3, k1 < 3, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_T2187 shape: fp32(1, 7, 7, 176):(8624, 1232, 176, 1):33.6875 KiB
// Elementwise op: [[pid(reduction_A_block_reduce_8)]] X_T2190 = div(X_T2187, X_T2189)
// Tile size: { 64, 3, 3, 7, 1 }
// Contraction output var shape: fp32(1, 7, 7, 176):(8624, 1232, 176, 1):33.6875 KiB
// Computed true ops: 232848
// Computed work groups: 21
// Computed inner loops: 1
// Computed shared mem: 11520
// Computed out regs: 2048
// Computed mem read: 11576
...
// lwork = 256, 1, 1
// gwork = 768, 7, 1
__kernel void kernel_c42_sdk_838(__global float* restrict  X_T2190, __global const float* restrict  in1, __global const float* restrict  X_T2187)
{
  ...
  for (int c_lid = 0; c_lid < 2; c_lid += 1)
  {
    ...
    if (c_cond)
    {
      ...
      if (x0_cond)
      {
        float LX_T2189 = agg[c_lid];
        int gout_idx = (((c_gid + c) + (1232 * x0_tid)) + (176 * x1_gid));
        float LX_T2187 = X_T2187[gout_idx];
        float LX_T2190 = (LX_T2187 / LX_T2189);
        X_T2190[gout_idx] = LX_T2190;
      }
    }
  }
}
