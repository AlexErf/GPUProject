...
// lid: 256 1 1
// Original:
// X_T1608[n, x0, x1, c : _T2535, _T2536, _T2537, _T2538] = +(X_T1607[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 3, k1 < 3
// With Index Variables Made Integral:
// X_T1608[n, x0, x1, c : _T2535, _T2536, _T2537, _T2538] = +(X_T1607[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 3, k1 < 3, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_T1606 shape: fp32(1, 21, 21, 336):(148176, 7056, 336, 1):578.812 KiB
// Elementwise op: [[pid(reduction_A_block_reduce_6)]] X_T1609 = div(X_T1606, X_T1608)
...
// lwork = 256, 1, 1
// gwork = 2816, 11, 1
__kernel void kernel_c42_sdk_612(__global float* restrict  X_T1609, __global const float* restrict  in1, __global const float* restrict  X_T1606)
{
    ...
    if (x1_cond)
    {
      for (int x0_lid = 0; x0_lid < 6; x0_lid += 1)
      {
        ...
        if (x0_cond)
        {
          ...
          float LX_T1608 = agg[x0_lid];
          int gout_idx = (((c_gid + c_tid) + (7056 * x0)) + (336 * (x1_gid + x1_tid)));
          float LX_T1606 = X_T1606[gout_idx];
          float LX_T1609 = (LX_T1606 / LX_T1608);
          X_T1609[gout_idx] = LX_T1609;
        }
      }
    }
  }
