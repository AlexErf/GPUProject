...
// lid: 256 1 1
// Original:
// X_T1576[n, x0, x1, c : _T2475, _T2476, _T2477, _T2478] = +(X_T1455[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3
// With Index Variables Made Integral:
// X_T1576[n, x0, x1, c : _T2475, _T2476, _T2477, _T2478] = +(X_T1455[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_T1443 shape: fp32(1, 14, 14, 88):(17248, 1232, 88, 1):67.375 KiB
// Elementwise op: [[pid(normal_A_block_5)]] X_T1577 = div(X_T1576, X_T1360)
// Elementwise op: [[pid(Add)]] X_T1578 = add(X_T1577, X_T1443)
...
// lwork = 256, 1, 1
// gwork = 768, 7, 1
__kernel void kernel_c42_sdk_595(__global float* restrict  X_T1578, __global const float* restrict  in1, __global const float* restrict  X_T1360, __global const float* restrict  X_T1443)
{
  in1 = (in1 + -1320);
  ...
  if (c_cond)
  {
    for (int x0_lid = 0; x0_lid < 4; x0_lid += 1)
    {
      ...
      if (x0_cond)
      {
        ...
        float LX_T1576 = agg[x0_lid];
        int gout_idx = (((c_gid + c_tid) + (1232 * x0)) + (88 * (x1_gid + x1_tid)));
        if (((gout_idx >= 0) && (gout_idx < 17248)))
        {
          float LX_T1360 = X_T1360[gout_idx];
          float LX_T1443 = X_T1443[gout_idx];
          float LX_T1577 = (LX_T1576 / LX_T1360);
          float LX_T1578 = (LX_T1577 + LX_T1443);
