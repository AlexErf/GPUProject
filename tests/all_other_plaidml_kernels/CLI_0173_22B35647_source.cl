...
// lid: 256 1 1
// Original:
// X_T817[n, x0, x1, c : _T1266, _T1267, _T1268, _T1269] = +(X_T468[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3
// With Index Variables Made Integral:
// X_T817[n, x0, x1, c : _T1266, _T1267, _T1268, _T1269] = +(X_T468[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise op: [[pid(normal_A_block_1)]] X_T818 = div(X_T817, X_T648)
// Elementwise op: [[pid(Add)]] X_T819 = add(X_T818, X_T818)
...
// lwork = 256, 1, 1
// gwork = 1792, 2, 1
__kernel void kernel_c42_sdk_299(__global float* restrict  X_T819, __global const float* restrict  in1, __global const float* restrict  X_T648)
{
  in1 = (in1 + -1276);
    ...
    if (x0_cond)
    {
      ...
      for (int c_lid = 0; c_lid < 2; c_lid += 1)
      {
        ...
        if (c_cond)
        {
          ...
          float LX_T817 = agg[(c_lid + (x0_lid * 2))];
          int gout_idx = ((c + (1232 * (x0_gid + x0))) + (44 * (x1_gid + x1_tid)));
          if (((gout_idx >= 0) && (gout_idx < 34496)))
          {
            float LX_T648 = X_T648[gout_idx];
            float LX_T818 = (LX_T817 / LX_T648);
            float LX_T819 = (LX_T818 + LX_T818);
            X_T819[gout_idx] = LX_T819;
          }
        }
      }
    }
