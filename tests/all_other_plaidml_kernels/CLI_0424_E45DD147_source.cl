...
// lid: 256 1 1
// Original:
// X_T1092[n, x0, x1, co : _T1514, _T1515, _T1516, _T1517] = +(X_T1091[n, k0 + x0, k1 + x1, ci] * X_I_409[k0, k1, ci, co])
// With Index Variables Made Integral:
// X_T1092[n, x0, x1, co : _T1514, _T1515, _T1516, _T1517] = +(X_T1091[n, k0 + x0, k1 + x1, ci] * X_I_409[k0, k1, ci, co]), 500000000 + ci < 1000000000, 500000000 + co < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise op: [[pid(Sub)]] X_T1093 = sub(X_T1092, X_I_408)
// Elementwise op: [[pid(Mul)]] X_T1094 = mul(X_T1093, X_I_407)
...
// lwork = 256, 1, 1
// gwork = 1024, 4, 1
__kernel void kernel_c124_sdk_363(__global float* restrict  X_T1094, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_408, __global const float* restrict  X_I_407)
{
  ...
  if (x1_cond)
  {
    for (int x0_lid = 0; x0_lid < 7; x0_lid += 1)
    {
      ...
      float LX_T1093 = (LX_T1092 - LX_I_408);
      float LX_T1094 = (LX_T1093 * LX_I_407);
      X_T1094[gout_idx] = LX_T1094;
    }
  }
}
