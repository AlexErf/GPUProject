...
// lid: 256 1 1
// Original:
// X_T2881[n, x0, x1, c : _T4583, _T4584, _T4585, _T4586] = +(X_T2880[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 3, k1 < 3
// With Index Variables Made Integral:
// X_T2881[n, x0, x1, c : _T4583, _T4584, _T4585, _T4586] = +(X_T2880[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 3, k1 < 3, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_T2879 shape: fp32(1, 11, 11, 672):(81312, 7392, 672, 1):317.625 KiB
// Elementwise op: [[pid(reduction_A_block_reduce_12)]] X_T2882 = div(X_T2879, X_T2881)
// Tile size: { 256, 3, 1, 11, 1 }
// Contraction output var shape: fp32(1, 11, 11, 672):(81312, 7392, 672, 1):317.625 KiB
// Computed true ops: 2195424
// Computed work groups: 33
// Computed inner loops: 3
// Computed shared mem: 23552
// Computed out regs: 16384
// Computed mem read: 23904
...
// lwork = 256, 1, 1
// gwork = 768, 11, 1
__kernel void kernel_c42_sdk_1114(__global float* restrict  X_T2882, __global const float* restrict  in1, __global const float* restrict  X_T2879)
{
    ...
    if (c_cond)
    {
      ...
      for (int x0_lid = 0; x0_lid < 2; x0_lid += 1)
      {
        ...
        if (x0_cond)
        {
          ...
          float LX_T2881 = agg[(c_lid + (x0_lid * 8))];
          int gout_idx = (((c_gid + c) + (7392 * x0)) + (672 * x1_gid));
          float LX_T2879 = X_T2879[gout_idx];
          float LX_T2882 = (LX_T2879 / LX_T2881);
          X_T2882[gout_idx] = LX_T2882;
        }
      }
    }
  }
