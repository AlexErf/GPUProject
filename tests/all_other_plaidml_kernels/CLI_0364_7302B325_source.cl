...
// lid: 256 1 1
// Original:
// X_T842[n, x0, x1, co : _T1144, _T1145, _T1146, _T1147] = +(X_T841[n, k0 + x0, k1 + x1, ci] * X_I_309[k0, k1, ci, co])
// With Index Variables Made Integral:
// X_T842[n, x0, x1, co : _T1144, _T1145, _T1146, _T1147] = +(X_T841[n, k0 + x0, k1 + x1, ci] * X_I_309[k0, k1, ci, co]), 500000000 + ci < 1000000000, 500000000 + co < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise op: [[pid(Sub)]] X_T843 = sub(X_T842, X_I_308)
// Elementwise op: [[pid(Mul)]] X_T844 = mul(X_T843, X_I_307)
...
// lwork = 256, 1, 1
// gwork = 1024, 4, 1
__kernel void kernel_c124_sdk_273(__global float* restrict  X_T844, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_308, __global const float* restrict  X_I_307)
{
  ...
  if (x1_cond)
  {
    for (int x0_lid = 0; x0_lid < 7; x0_lid += 1)
    {
      ...
      float LX_T843 = (LX_T842 - LX_I_308);
      float LX_T844 = (LX_T843 * LX_I_307);
      X_T844[gout_idx] = LX_T844;
    }
  }
}
