...
// lid: 256 1 1
// Original:
// X_T151[n, x0, x1, co : _T119, _T120, _T121, _T122] = +(X_T150[n, k0 + x0, k1 + x1, ci] * X_I_47[k0, k1, ci, co])
// With Index Variables Made Integral:
// X_T151[n, x0, x1, co : _T119, _T120, _T121, _T122] = +(X_T150[n, k0 + x0, k1 + x1, ci] * X_I_47[k0, k1, ci, co]), 500000000 + ci < 1000000000, 500000000 + co < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise op: [[pid(Sub)]] X_T152 = sub(X_T151, X_I_46)
// Elementwise op: [[pid(Mul)]] X_T153 = mul(X_T152, X_I_45)
// Tile size: { 32, 64, 8, 8 }
...
// lwork = 256, 1, 1
// gwork = 1792, 7, 2
__kernel void kernel_c124_sdk_27(__global float* restrict  X_T153, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_46, __global const float* restrict  X_I_45)
{
  ...
  for (int co_lid = 0; co_lid < 2; co_lid += 1)
  {
    ...
    for (int x1_lid = 0; x1_lid < 2; x1_lid += 1)
    {
      ...
      {
        int x0 = ((2 * x0_lid) + x0_tid);
        float LX_T151 = agg[((co_lid + (x1_lid * 2)) + (x0_lid * 4))];
        int gout_idx = (((co_gid + co) + (7168 * (x0_gid + x0))) + (128 * (x1_gid + x1)));
        float LX_I_46 = X_I_46[(co_gid + co)];
        float LX_I_45 = X_I_45[(co_gid + co)];
        float LX_T152 = (LX_T151 - LX_I_46);
        float LX_T153 = (LX_T152 * LX_I_45);
        X_T153[gout_idx] = LX_T153;
      }
    }
  }
}
