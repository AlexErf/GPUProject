...
// lid: 256 1 1
// Original:
// X_T2344[n, x0, x1, c : _T3716, _T3717, _T3718, _T3719] = +(X_T2343[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 1, k1 < 1
// With Index Variables Made Integral:
// X_T2344[n, x0, x1, c : _T3716, _T3717, _T3718, _T3719] = +(X_T2343[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 1, k1 < 1, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_T2342 shape: fp32(1, 7, 7, 528):(25872, 3696, 528, 1):101.062 KiB
// Elementwise op: [[pid(normal_A_block_9)]] X_T2345 = div(X_T2342, X_T2344)
// Tile size: { 528, 1, 1 }
// Contraction output var shape: fp32(1, 7, 7, 528):(25872, 3696, 528, 1):101.062 KiB
// Computed true ops: 77616
// Computed work groups: 49
// Computed inner loops: 1
// Computed shared mem: 2112
// Computed out regs: 3072
// Computed mem read: 2116
...
// lwork = 256, 1, 1
// gwork = 1792, 7, 1
__kernel void kernel_c42_sdk_898(__global float* restrict  X_T2345, __global const float* restrict  in1, __global const float* restrict  X_T2342)
{
  ...
  for (int c_lid = 0; c_lid < 3; c_lid += 1)
  {
    ...
    {
      int c = ((256 * c_lid) + c_tid);
      float LX_T2344 = agg[c_lid];
      int gout_idx = ((c + (3696 * x0_gid)) + (528 * x1_gid));
      float LX_T2342 = X_T2342[gout_idx];
      float LX_T2345 = (LX_T2342 / LX_T2344);
      X_T2345[gout_idx] = LX_T2345;
    }
  }
}
