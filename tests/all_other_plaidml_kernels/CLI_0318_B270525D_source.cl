...
// lid: 256 1 1
// Original:
// X_T734[n, x0, x1, co : _T996, _T997, _T998, _T999] = +(X_T733[n, k0 + x0, k1 + x1, ci] * X_I_269[k0, k1, ci, co])
// With Index Variables Made Integral:
// X_T734[n, x0, x1, co : _T996, _T997, _T998, _T999] = +(X_T733[n, k0 + x0, k1 + x1, ci] * X_I_269[k0, k1, ci, co]), 500000000 + ci < 1000000000, 500000000 + co < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise op: [[pid(Sub)]] X_T735 = sub(X_T734, X_I_268)
// Elementwise op: [[pid(Mul)]] X_T736 = mul(X_T735, X_I_267)
...
// lwork = 256, 1, 1
// gwork = 1024, 4, 1
__kernel void kernel_c108_sdk_237(__global float* restrict  X_T736, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_268, __global const float* restrict  X_I_267)
{
  ...
  if (x1_cond)
  {
    for (int x0_lid = 0; x0_lid < 7; x0_lid += 1)
    {
      ...
      float LX_T735 = (LX_T734 - LX_I_268);
      float LX_T736 = (LX_T735 * LX_I_267);
      X_T736[gout_idx] = LX_T736;
    }
  }
}
