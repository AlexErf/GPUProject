...
// lid: 256 1 1
// Original:
// X_T2833[n, x0, x1, c : _T4503, _T4504, _T4505, _T4506] = >(X_T2832[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 3, k1 < 3
// With Index Variables Made Integral:
// X_T2833[n, x0, x1, c : _T4503, _T4504, _T4505, _T4506] = >(X_T2832[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 3, k1 < 3, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// lwork = 256, 1, 1
// gwork = 768, 11, 1
__kernel void kernel_c42_sdk_1096(__global float* restrict  X_T2833, __global const float* restrict  in1)
{
    ...
    if (c_cond)
    {
      ...
      for (int x0_lid = 0; x0_lid < 2; x0_lid += 1)
      {
        ...
        if (x0_cond)
        {
          ...
          float LX_T2833 = agg[(c_lid + (x0_lid * 8))];
          LX_T2833 = select((float)LX_T2833, (float)0, (int)(LX_T2833 == (float)-FLT_MAX));
          int gout_idx = (((c_gid + c) + (7392 * x0)) + (672 * x1_gid));
          X_T2833[gout_idx] = LX_T2833;
        }
      }
    }
  }
