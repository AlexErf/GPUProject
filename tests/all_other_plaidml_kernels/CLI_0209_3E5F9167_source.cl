...
// lid: 256 1 1
// Original:
// X_T247[n, x0, x1, c : _T278, _T279, _T280, _T281] = +(X_T246[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 2, k1 < 2
// With Index Variables Made Integral:
// X_T247[n, x0, x1, c : _T278, _T279, _T280, _T281] = +(X_T246[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 2, k1 < 2, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_T244 shape: fp32(1, 28, 28, 128):(100352, 3584, 128, 1):392 KiB
// Elementwise input X_I_91 shape: fp32(128):(1):512 bytes
// Elementwise input X_I_90 shape: fp32(128):(1):512 bytes
// Elementwise op: [[pid(pool2_pool)]] X_T248 = div(X_T244, X_T247)
// Elementwise op: [[pid(Sub)]] X_T254 = sub(X_T248, X_I_91)
// Elementwise op: [[pid(Mul)]] X_T255 = mul(X_T254, X_I_90)
...
// lwork = 256, 1, 1
// gwork = 7168, 4, 1
__kernel void kernel_c108_sdk_65(__global float* restrict  X_T248, __global float* restrict  X_T255, __global const float* restrict  in1, __global const float* restrict  X_T244, __global const float* restrict  X_I_91, __global const float* restrict  X_I_90)
{
  ...
  for (int x0_lid = 0; x0_lid < 4; x0_lid += 1)
  {
    ...
    if (x0_cond)
    {
      ...
      float LX_T248 = (LX_T244 / LX_T247);
      float LX_T254 = (LX_T248 - LX_I_91);
      float LX_T255 = (LX_T254 * LX_I_90);
      X_T248[gout_idx] = LX_T248;
      X_T255[gout_idx] = LX_T255;
    }
  }
}
