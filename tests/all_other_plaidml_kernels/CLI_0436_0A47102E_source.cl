...
// lid: 256 1 1
// Original:
// X_T1142[n, x0, x1, co : _T1588, _T1589, _T1590, _T1591] = +(X_T1141[n, k0 + x0, k1 + x1, ci] * X_I_429[k0, k1, ci, co])
// With Index Variables Made Integral:
// X_T1142[n, x0, x1, co : _T1588, _T1589, _T1590, _T1591] = +(X_T1141[n, k0 + x0, k1 + x1, ci] * X_I_429[k0, k1, ci, co]), 500000000 + ci < 1000000000, 500000000 + co < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise op: [[pid(Sub)]] X_T1143 = sub(X_T1142, X_I_428)
// Elementwise op: [[pid(Mul)]] X_T1144 = mul(X_T1143, X_I_427)
...
// lwork = 256, 1, 1
// gwork = 1024, 4, 1
__kernel void kernel_c124_sdk_381(__global float* restrict  X_T1144, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_428, __global const float* restrict  X_I_427)
{
  ...
  if (x1_cond)
  {
    for (int x0_lid = 0; x0_lid < 7; x0_lid += 1)
    {
      ...
      float LX_T1143 = (LX_T1142 - LX_I_428);
      float LX_T1144 = (LX_T1143 * LX_I_427);
      X_T1144[gout_idx] = LX_T1144;
    }
  }
}
