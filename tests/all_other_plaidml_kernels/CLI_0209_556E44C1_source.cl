...
// lid: 256 1 1
// Original:
// X_T1763[n, x0, x1, c : _T2780, _T2781, _T2782, _T2783] = +(X_T1762[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 1, k1 < 1
// With Index Variables Made Integral:
// X_T1763[n, x0, x1, c : _T2780, _T2781, _T2782, _T2783] = +(X_T1762[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 1, k1 < 1, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_T1761 shape: fp32(1, 21, 21, 1008):(444528, 21168, 1008, 1):1736.44 KiB
// Elementwise op: [[pid(normal_A_block_7)]] X_T1764 = div(X_T1761, X_T1763)
// Tile size: { 1008, 1, 1 }
// Contraction output var shape: fp32(1, 21, 21, 1008):(444528, 21168, 1008, 1):1736.44 KiB
// Computed true ops: 1333584
...
// lwork = 256, 1, 1
// gwork = 5376, 21, 1
__kernel void kernel_c42_sdk_672(__global float* restrict  X_T1764, __global const float* restrict  in1, __global const float* restrict  X_T1761)
{
  ...
  for (int c_lid = 0; c_lid < 4; c_lid += 1)
  {
    ...
    {
      int c = ((256 * c_lid) + c_tid);
      float LX_T1763 = agg[c_lid];
      int gout_idx = ((c + (21168 * x0_gid)) + (1008 * x1_gid));
      float LX_T1761 = X_T1761[gout_idx];
      float LX_T1764 = (LX_T1761 / LX_T1763);
      X_T1764[gout_idx] = LX_T1764;
    }
  }
}
