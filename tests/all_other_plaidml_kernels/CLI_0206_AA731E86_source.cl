...
// lid: 256 1 1
// Original:
// X_T242[n, x0, x1, co : _T263, _T264, _T265, _T266] = +(X_T241[n, k0 + x0, k1 + x1, ci] * X_I_13[k0, k1, ci, co])
// With Index Variables Made Integral:
// X_T242[n, x0, x1, co : _T263, _T264, _T265, _T266] = +(X_T241[n, k0 + x0, k1 + x1, ci] * X_I_13[k0, k1, ci, co]), 500000000 + ci < 1000000000, 500000000 + co < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// lwork = 256, 1, 1
// gwork = 1792, 7, 2
__kernel void kernel_c108_sdk_62(__global float* restrict  X_T242, __global const float* restrict  in1, __global const float* restrict  in2)
{
  ...
  for (int co_lid = 0; co_lid < 2; co_lid += 1)
  {
    ...
    for (int x1_lid = 0; x1_lid < 2; x1_lid += 1)
    {
      ...
      {
        int x0 = ((2 * x0_lid) + x0_tid);
        float LX_T242 = agg[((co_lid + (x1_lid * 2)) + (x0_lid * 4))];
