...
// lid: 256 1 1
// Original:
// X_T464[n, x0, x1, co : _T595, _T596, _T597, _T598] = +(X_T463[n, k0 + x0, k1 + x1, ci] * X_I_168[k0, k1, ci, co])
// With Index Variables Made Integral:
// X_T464[n, x0, x1, co : _T595, _T596, _T597, _T598] = +(X_T463[n, k0 + x0, k1 + x1, ci] * X_I_168[k0, k1, ci, co]), 500000000 + ci < 1000000000, 500000000 + co < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise op: [[pid(Sub)]] X_T465 = sub(X_T464, X_I_167)
// Elementwise op: [[pid(Mul)]] X_T466 = mul(X_T465, X_I_166)
...
// lwork = 256, 1, 1
// gwork = 1792, 2, 2
__kernel void kernel_c108_sdk_141(__global float* restrict  X_T466, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_167, __global const float* restrict  X_I_166)
{
  ...
  for (int x1_lid = 0; x1_lid < 4; x1_lid += 1)
  {
    ...
    if (x1_cond)
    {
      ...
      for (int co_lid = 0; co_lid < 2; co_lid += 1)
      {
        ...
        {
          int x0 = ((2 * x0_lid) + x0_tid);
          float LX_T464 = agg[((co_lid + (x1_lid * 2)) + (x0_lid * 8))];
          int gout_idx = (((co_gid + co) + (3584 * (x0_gid + x0))) + (128 * (x1_gid + x1)));
          float LX_I_167 = X_I_167[(co_gid + co)];
          float LX_I_166 = X_I_166[(co_gid + co)];
          float LX_T465 = (LX_T464 - LX_I_167);
          float LX_T466 = (LX_T465 * LX_I_166);
          X_T466[gout_idx] = LX_T466;
        }
      }
    }
  }
