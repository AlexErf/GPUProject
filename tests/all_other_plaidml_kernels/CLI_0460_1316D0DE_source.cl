...
// lid: 256 1 1
// Original:
// X_T1334[n, x0, x1, co : _T1884, _T1885, _T1886, _T1887] = +(X_T1333[n, k0 + x0, k1 + x1, ci] * X_I_509[k0, k1, ci, co])
// With Index Variables Made Integral:
// X_T1334[n, x0, x1, co : _T1884, _T1885, _T1886, _T1887] = +(X_T1333[n, k0 + x0, k1 + x1, ci] * X_I_509[k0, k1, ci, co]), 500000000 + ci < 1000000000, 500000000 + co < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise op: [[pid(Sub)]] X_T1335 = sub(X_T1334, X_I_508)
// Elementwise op: [[pid(Mul)]] X_T1336 = mul(X_T1335, X_I_507)
...
// lwork = 256, 1, 1
// gwork = 1024, 4, 1
__kernel void kernel_c108_sdk_453(__global float* restrict  X_T1336, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_508, __global const float* restrict  X_I_507)
{
  ...
  if (x1_cond)
  {
    for (int x0_lid = 0; x0_lid < 7; x0_lid += 1)
    {
      ...
      float LX_T1335 = (LX_T1334 - LX_I_508);
      float LX_T1336 = (LX_T1335 * LX_I_507);
      X_T1336[gout_idx] = LX_T1336;
    }
  }
}
