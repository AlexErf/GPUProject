...
// lid: 256 1 1
// Original:
// X_T76[n, x0, x1, c : _T25, _T26, _T27, _T28] = >(X_T73[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 3, k1 < 3
// With Index Variables Made Integral:
// X_T76[n, x0, x1, c : _T25, _T26, _T27, _T28] = >(X_T73[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 3, k1 < 3, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise op: [[pid(Sub)]] X_T83 = sub(X_T76, X_I_30)
// Elementwise op: [[pid(Mul)]] X_T84 = mul(X_T83, X_I_29)
...
// lwork = 256, 1, 1
// gwork = 1792, 28, 1
__kernel void kernel_c108_sdk_5(__global float* restrict  X_T76, __global float* restrict  X_T84, __global const float* restrict  in1, __global const float* restrict  X_I_30, __global const float* restrict  X_I_29)
{
  ...
  for (int c_lid = 0; c_lid < 2; c_lid += 1)
  {
    ...
    {
      int x0 = ((4 * x0_lid) + x0_tid);
      float LX_T76 = agg[(c_lid + (x0_lid * 2))];
      LX_T76 = select((float)LX_T76, (float)0, (int)(LX_T76 == (float)-FLT_MAX));
      int gout_idx = ((c + (3584 * (x0_gid + x0))) + (64 * (x1_gid + x1_tid)));
      float LX_I_30 = X_I_30[c];
      float LX_I_29 = X_I_29[c];
      float LX_T83 = (LX_T76 - LX_I_30);
      float LX_T84 = (LX_T83 * LX_I_29);
      X_T76[gout_idx] = LX_T76;
