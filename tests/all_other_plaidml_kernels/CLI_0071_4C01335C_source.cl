...
// lid: 256 1 1
// Original:
// X_T56[n, x0, x1, co : _T26, _T27, _T28, _T29] = +(X_T55[n, k0 + 2*x0, k1 + 2*x1, ci] * X_I_89[k0, k1, ci, co])
// With Index Variables Made Integral:
// X_T56[n, x0, x1, co : _T26, _T27, _T28, _T29] = +(X_T55[n, k0 + 2*x0, k1 + 2*x1, ci] * X_I_89[k0, k1, ci, co]), 500000000 + ci < 1000000000, 500000000 + co < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_I_88 shape: fp32(32):(1):128 bytes
// Elementwise input X_I_87 shape: fp32(32):(1):128 bytes
// Elementwise op: [[pid(Sub)]] X_T57 = sub(X_T56, X_I_88)
// Elementwise op: [[pid(Mul)]] X_T58 = mul(X_T57, X_I_87)
...
// lwork = 256, 1, 1
// gwork = 1792, 14, 1
__kernel void kernel_c43_sdk_8(__global float* restrict  X_T58, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_88, __global const float* restrict  X_I_87)
{
  ...
  for (int x1_lid = 0; x1_lid < 2; x1_lid += 1)
  {
    ...
    {
      int x0 = ((2 * x0_lid) + x0_tid);
      float LX_T56 = agg[(x1_lid + (x0_lid * 2))];
      int gout_idx = ((co_tid + (3584 * (x0_gid + x0))) + (32 * (x1_gid + x1)));
      float LX_I_88 = X_I_88[co_tid];
      float LX_I_87 = X_I_87[co_tid];
      float LX_T57 = (LX_T56 - LX_I_88);
      float LX_T58 = (LX_T57 * LX_I_87);
      X_T58[gout_idx] = LX_T58;
    }
  }
}
