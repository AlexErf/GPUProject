...
// lid: 256 1 1
// Original:
// X_T1360[n, x0, x1, c : _T2133, _T2134, _T2135, _T2136] = +(X_T1359[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3
// With Index Variables Made Integral:
// X_T1360[n, x0, x1, c : _T2133, _T2134, _T2135, _T2136] = +(X_T1359[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_T1358 shape: fp32(1, 14, 14, 88):(17248, 1232, 88, 1):67.375 KiB
// Elementwise input X_T1257 shape: fp32(1, 14, 14, 88):(17248, 1232, 88, 1):67.375 KiB
// Elementwise op: [[pid(reduction_A_block_reduce_4)]] X_T1361 = div(X_T1358, X_T1360)
// Elementwise op: [[pid(Add)]] X_T1362 = add(X_T1257, X_T1361)
...
// lwork = 256, 1, 1
// gwork = 768, 7, 1
__kernel void kernel_c42_sdk_511(__global float* restrict  X_T1360, __global float* restrict  X_T1362, __global const float* restrict  in1, __global const float* restrict  X_T1358, __global const float* restrict  X_T1257)
{
  in1 = (in1 + -1320);
  ...
  if (c_cond)
  {
    for (int x0_lid = 0; x0_lid < 4; x0_lid += 1)
    {
      ...
      if (x0_cond)
      {
        ...
        float LX_T1360 = agg[x0_lid];
        int gout_idx = (((c_gid + c_tid) + (1232 * x0)) + (88 * (x1_gid + x1_tid)));
        if (((gout_idx >= 0) && (gout_idx < 17248)))
        {
          float LX_T1358 = X_T1358[gout_idx];
          float LX_T1257 = X_T1257[gout_idx];
          float LX_T1361 = (LX_T1358 / LX_T1360);
          float LX_T1362 = (LX_T1257 + LX_T1361);
          X_T1360[gout_idx] = LX_T1360;
