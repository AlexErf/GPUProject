...
// lid: 256 1 1
// Original:
// X_T934[n, x0, x1, co : _T1292, _T1293, _T1294, _T1295] = +(X_T933[n, k0 + x0, k1 + x1, ci] * X_I_349[k0, k1, ci, co])
// With Index Variables Made Integral:
// X_T934[n, x0, x1, co : _T1292, _T1293, _T1294, _T1295] = +(X_T933[n, k0 + x0, k1 + x1, ci] * X_I_349[k0, k1, ci, co]), 500000000 + ci < 1000000000, 500000000 + co < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise op: [[pid(Sub)]] X_T935 = sub(X_T934, X_I_348)
// Elementwise op: [[pid(Mul)]] X_T936 = mul(X_T935, X_I_347)
...
// lwork = 256, 1, 1
// gwork = 1024, 4, 1
__kernel void kernel_c108_sdk_309(__global float* restrict  X_T936, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_348, __global const float* restrict  X_I_347)
{
  ...
  if (x1_cond)
  {
    for (int x0_lid = 0; x0_lid < 7; x0_lid += 1)
    {
      ...
      float LX_T935 = (LX_T934 - LX_I_348);
      float LX_T936 = (LX_T935 * LX_I_347);
      X_T936[gout_idx] = LX_T936;
    }
  }
}
