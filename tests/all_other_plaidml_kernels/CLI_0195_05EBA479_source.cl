...
// lid: 256 1 1
// Original:
// X_T193[n, x0, x1, co : _T193, _T194, _T195, _T196] = +(X_T192[n, k0 + x0, k1 + x1, ci] * X_I_67[k0, k1, ci, co])
// With Index Variables Made Integral:
// X_T193[n, x0, x1, co : _T193, _T194, _T195, _T196] = +(X_T192[n, k0 + x0, k1 + x1, ci] * X_I_67[k0, k1, ci, co]), 500000000 + ci < 1000000000, 500000000 + co < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise op: [[pid(Sub)]] X_T194 = sub(X_T193, X_I_66)
// Elementwise op: [[pid(Mul)]] X_T195 = mul(X_T194, X_I_65)
// Tile size: { 32, 64, 8, 8 }
...
// lwork = 256, 1, 1
// gwork = 1792, 7, 2
__kernel void kernel_c108_sdk_45(__global float* restrict  X_T195, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_66, __global const float* restrict  X_I_65)
{
  ...
  for (int co_lid = 0; co_lid < 2; co_lid += 1)
  {
    ...
    for (int x1_lid = 0; x1_lid < 2; x1_lid += 1)
    {
      ...
      {
        int x0 = ((2 * x0_lid) + x0_tid);
        float LX_T193 = agg[((co_lid + (x1_lid * 2)) + (x0_lid * 4))];
        int gout_idx = (((co_gid + co) + (7168 * (x0_gid + x0))) + (128 * (x1_gid + x1)));
        float LX_I_66 = X_I_66[(co_gid + co)];
        float LX_I_65 = X_I_65[(co_gid + co)];
        float LX_T194 = (LX_T193 - LX_I_66);
        float LX_T195 = (LX_T194 * LX_I_65);
        X_T195[gout_idx] = LX_T195;
      }
    }
  }
}
