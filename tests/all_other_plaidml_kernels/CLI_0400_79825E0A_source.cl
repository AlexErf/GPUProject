...
// lid: 256 1 1
// Original:
// X_T992[n, x0, x1, co : _T1366, _T1367, _T1368, _T1369] = +(X_T991[n, k0 + x0, k1 + x1, ci] * X_I_369[k0, k1, ci, co])
// With Index Variables Made Integral:
// X_T992[n, x0, x1, co : _T1366, _T1367, _T1368, _T1369] = +(X_T991[n, k0 + x0, k1 + x1, ci] * X_I_369[k0, k1, ci, co]), 500000000 + ci < 1000000000, 500000000 + co < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise op: [[pid(Sub)]] X_T993 = sub(X_T992, X_I_368)
// Elementwise op: [[pid(Mul)]] X_T994 = mul(X_T993, X_I_367)
...
// lwork = 256, 1, 1
// gwork = 1024, 4, 1
__kernel void kernel_c124_sdk_327(__global float* restrict  X_T994, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_368, __global const float* restrict  X_I_367)
{
  ...
  if (x1_cond)
  {
    for (int x0_lid = 0; x0_lid < 7; x0_lid += 1)
    {
      ...
      float LX_T993 = (LX_T992 - LX_I_368);
      float LX_T994 = (LX_T993 * LX_I_367);
      X_T994[gout_idx] = LX_T994;
    }
  }
}
