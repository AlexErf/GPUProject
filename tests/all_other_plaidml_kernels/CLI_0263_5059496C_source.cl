...
// lid: 256 1 1
// Original:
// X_T3031[n, x0, x1, c : _T4828, _T4829, _T4830, _T4831] = +(X_T3030[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 1, k1 < 1
// With Index Variables Made Integral:
// X_T3031[n, x0, x1, c : _T4828, _T4829, _T4830, _T4831] = +(X_T3030[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 1, k1 < 1, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_T3029 shape: fp32(1, 11, 11, 2016):(243936, 22176, 2016, 1):952.875 KiB
// Elementwise op: [[pid(normal_A_block_13)]] X_T3032 = div(X_T3029, X_T3031)
// Tile size: { 2016, 1, 1 }
// Contraction output var shape: fp32(1, 11, 11, 2016):(243936, 22176, 2016, 1):952.875 KiB
// Computed true ops: 731808
// Computed work groups: 121
// Computed inner loops: 1
// Computed shared mem: 8064
// Computed out regs: 8192
// Computed mem read: 8316
...
// lwork = 256, 1, 1
// gwork = 2816, 11, 1
__kernel void kernel_c42_sdk_1174(__global float* restrict  X_T3032, __global const float* restrict  in1, __global const float* restrict  X_T3029)
{
  ...
  for (int c_lid = 0; c_lid < 8; c_lid += 1)
  {
    ...
    {
      int c = ((256 * c_lid) + c_tid);
      float LX_T3031 = agg[c_lid];
      int gout_idx = ((c + (22176 * x0_gid)) + (2016 * x1_gid));
      float LX_T3029 = X_T3029[gout_idx];
      float LX_T3032 = (LX_T3029 / LX_T3031);
      X_T3032[gout_idx] = LX_T3032;
    }
  }
}
