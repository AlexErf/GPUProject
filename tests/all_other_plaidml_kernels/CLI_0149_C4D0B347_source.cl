...
// lid: 256 1 1
// Original:
// X_T227[n, x0, x1, c : _T278, _T279, _T280, _T281] = +(X_T226[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 2, k1 < 2
// With Index Variables Made Integral:
// X_T227[n, x0, x1, c : _T278, _T279, _T280, _T281] = +(X_T226[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 2, k1 < 2, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_T224 shape: fp32(1, 28, 28, 128):(100352, 3584, 128, 1):392 KiB
// Elementwise input X_I_91 shape: fp32(128):(1):512 bytes
// Elementwise input X_I_90 shape: fp32(128):(1):512 bytes
// Elementwise op: [[pid(pool2_pool)]] X_T228 = div(X_T224, X_T227)
// Elementwise op: [[pid(Sub)]] X_T234 = sub(X_T228, X_I_91)
// Elementwise op: [[pid(Mul)]] X_T235 = mul(X_T234, X_I_90)
...
// lwork = 256, 1, 1
// gwork = 7168, 4, 1
__kernel void kernel_c68_sdk_65(__global float* restrict  X_T228, __global float* restrict  X_T235, __global const float* restrict  in1, __global const float* restrict  X_T224, __global const float* restrict  X_I_91, __global const float* restrict  X_I_90)
{
  ...
  for (int x0_lid = 0; x0_lid < 4; x0_lid += 1)
  {
    ...
    if (x0_cond)
    {
      ...
      float LX_T228 = (LX_T224 / LX_T227);
      float LX_T234 = (LX_T228 - LX_I_91);
      float LX_T235 = (LX_T234 * LX_I_90);
      X_T228[gout_idx] = LX_T228;
      X_T235[gout_idx] = LX_T235;
    }
  }
}
