...
// lid: 256 1 1
// Original:
// X_T157[n, x0, x1, co : _T142, _T143, _T144, _T145] = +(X_T156[n, k0 + x0, k1 + x1, ci] * X_I_86[k0, k1, ci, co])
// With Index Variables Made Integral:
// X_T157[n, x0, x1, co : _T142, _T143, _T144, _T145] = +(X_T156[n, k0 + x0, k1 + x1, ci] * X_I_86[k0, k1, ci, co]), 500000000 + ci < 1000000000, 500000000 + co < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_I_85 shape: fp32(128):(1):512 bytes
// Elementwise input X_I_84 shape: fp32(128):(1):512 bytes
// Elementwise op: [[pid(Sub)]] X_T158 = sub(X_T157, X_I_85)
// Elementwise op: [[pid(Mul)]] X_T159 = mul(X_T158, X_I_84)
// Tile size: { 32, 64, 8, 8 }
...
// lwork = 256, 1, 1
// gwork = 1792, 7, 2
__kernel void kernel_c25_sdk_37(__global float* restrict  X_T159, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_85, __global const float* restrict  X_I_84)
{
  ...
  for (int co_lid = 0; co_lid < 2; co_lid += 1)
  {
    ...
    for (int x1_lid = 0; x1_lid < 2; x1_lid += 1)
    {
      ...
      {
        int x0 = ((2 * x0_lid) + x0_tid);
        float LX_T157 = agg[((co_lid + (x1_lid * 2)) + (x0_lid * 4))];
        int gout_idx = (((co_gid + co) + (7168 * (x0_gid + x0))) + (128 * (x1_gid + x1)));
        float LX_I_85 = X_I_85[(co_gid + co)];
        float LX_I_84 = X_I_84[(co_gid + co)];
        float LX_T158 = (LX_T157 - LX_I_85);
        float LX_T159 = (LX_T158 * LX_I_84);
