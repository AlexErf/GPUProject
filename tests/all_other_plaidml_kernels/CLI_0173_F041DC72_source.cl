...
// lid: 256 1 1
// Original:
// X_T816[n, x0, x1, c : _T1266, _T1267, _T1268, _T1269] = +(X_T469[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3
// With Index Variables Made Integral:
// X_T816[n, x0, x1, c : _T1266, _T1267, _T1268, _T1269] = +(X_T469[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise op: [[pid(normal_A_block_1)]] X_T817 = div(X_T816, X_T647)
// Elementwise op: [[pid(Add)]] X_T818 = add(X_T817, X_T817)
...
// lwork = 256, 1, 1
// gwork = 768, 3, 11
__kernel void kernel_c42_sdk_299(__global float* restrict  X_T818, __global const float* restrict  in1, __global const float* restrict  X_T647)
{
  in1 = (in1 + -7224);
      ...
      for (int x1_lid = 0; x1_lid < 4; x1_lid += 1)
      {
        ...
        if (x1_cond)
        {
          ...
          for (int x0_lid = 0; x0_lid < 2; x0_lid += 1)
          {
            ...
            {
              int x0 = ((2 * x0_lid) + x0_tid);
              float LX_T816 = agg[((c_lid + (x1_lid * 2)) + (x0_lid * 8))];
              int gout_idx = (((c_gid + c) + (7056 * (x0_gid + x0))) + (168 * (x1_gid + x1)));
              if (((gout_idx >= 0) && (gout_idx < 296352)))
              {
                float LX_T647 = X_T647[gout_idx];
                float LX_T817 = (LX_T816 / LX_T647);
                float LX_T818 = (LX_T817 + LX_T817);
                X_T818[gout_idx] = LX_T818;
              }
            }
          }
        }
