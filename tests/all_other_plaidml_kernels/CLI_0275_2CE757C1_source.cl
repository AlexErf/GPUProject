...
// lid: 256 1 1
// Original:
// X_T2510[n, x0, x1, c : _T3989, _T3990, _T3991, _T3992] = +(X_T2509[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3
// With Index Variables Made Integral:
// X_T2510[n, x0, x1, c : _T3989, _T3990, _T3991, _T3992] = +(X_T2509[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_T2508 shape: fp32(1, 7, 7, 176):(8624, 1232, 176, 1):33.6875 KiB
// Elementwise op: [[pid(normal_A_block_9)]] X_T2511 = div(X_T2508, X_T2510)
// Elementwise op: [[pid(Add)]] X_T2512 = add(X_T2511, X_T2511)
...
// lwork = 256, 1, 1
// gwork = 768, 7, 1
__kernel void kernel_c42_sdk_964(__global float* restrict  X_T2512, __global const float* restrict  in1, __global const float* restrict  X_T2508)
{
  in1 = (in1 + -1408);
  ...
  for (int c_lid = 0; c_lid < 2; c_lid += 1)
  {
    ...
    if (c_cond)
    {
      ...
      if (x0_cond)
      {
        float LX_T2510 = agg[c_lid];
        int gout_idx = (((c_gid + c) + (1232 * x0_tid)) + (176 * x1_gid));
        if (((gout_idx >= 0) && (gout_idx < 8624)))
        {
          float LX_T2508 = X_T2508[gout_idx];
          float LX_T2511 = (LX_T2508 / LX_T2510);
          float LX_T2512 = (LX_T2511 + LX_T2511);
          X_T2512[gout_idx] = LX_T2512;
        }
      }
    }
  }
