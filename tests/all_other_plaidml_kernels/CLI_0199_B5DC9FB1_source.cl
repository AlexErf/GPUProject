...
// lid: 256 1 1
// Original:
// X_T1707[n, x0, x1, c : _T2689, _T2690, _T2691, _T2692] = +(X_T1706[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3
// With Index Variables Made Integral:
// X_T1707[n, x0, x1, c : _T2689, _T2690, _T2691, _T2692] = +(X_T1706[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_T1705 shape: fp32(1, 21, 21, 336):(148176, 7056, 336, 1):578.812 KiB
// Elementwise input X_T1604 shape: fp32(1, 21, 21, 336):(148176, 7056, 336, 1):578.812 KiB
// Elementwise op: [[pid(reduction_A_block_reduce_6)]] X_T1708 = div(X_T1705, X_T1707)
// Elementwise op: [[pid(Add)]] X_T1709 = add(X_T1604, X_T1708)
...
// lwork = 256, 1, 1
// gwork = 1536, 11, 1
__kernel void kernel_c42_sdk_649(__global float* restrict  X_T1707, __global float* restrict  X_T1709, __global const float* restrict  in1, __global const float* restrict  X_T1705, __global const float* restrict  X_T1604)
{
  in1 = (in1 + -7392);
    ...
    if (x1_cond)
    {
      for (int x0_lid = 0; x0_lid < 11; x0_lid += 1)
      {
        ...
        if (x0_cond)
        {
          ...
          float LX_T1707 = agg[x0_lid];
          int gout_idx = (((c_gid + c_tid) + (7056 * x0)) + (336 * (x1_gid + x1_tid)));
          if (((gout_idx >= 0) && (gout_idx < 148176)))
          {
            float LX_T1705 = X_T1705[gout_idx];
            float LX_T1604 = X_T1604[gout_idx];
            float LX_T1708 = (LX_T1705 / LX_T1707);
            float LX_T1709 = (LX_T1604 + LX_T1708);
            X_T1707[gout_idx] = LX_T1707;
            X_T1709[gout_idx] = LX_T1709;
          }
        }
      }
    }
