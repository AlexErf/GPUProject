...
// lid: 256 1 1
// Original:
// X_T63[n, x0, x1, c : _T69, _T70, _T71, _T72] = >(X_T62[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 2, k1 < 2
// With Index Variables Made Integral:
// X_T63[n, x0, x1, c : _T69, _T70, _T71, _T72] = >(X_T62[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 2, k1 < 2, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// lwork = 256, 1, 1
// gwork = 7168, 2, 1
__kernel void kernel_c18_sdk_10(__global float* restrict  X_T63, __global const float* restrict  in1)
{
  ...
  for (int x0_lid = 0; x0_lid < 4; x0_lid += 1)
  {
    ...
    if (x0_cond)
    {
      ...
      {
        int c = ((32 * c_lid) + c_tid);
        float LX_T63 = agg[(c_lid + (x0_lid * 4))];
        LX_T63 = select((float)LX_T63, (float)0, (int)(LX_T63 == (float)-FLT_MAX));
        int gout_idx = (((c_gid + c) + (7168 * x0)) + (256 * x1_gid));
        X_T63[gout_idx] = LX_T63;
      }
    }
  }
}
