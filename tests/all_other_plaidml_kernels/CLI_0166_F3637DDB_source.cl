...
// lid: 256 1 1
// Original:
// X_T655[n, x0, x1, c : _T1006, _T1007, _T1008, _T1009] = +(X_T654[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3
// With Index Variables Made Integral:
// X_T655[n, x0, x1, c : _T1006, _T1007, _T1008, _T1009] = +(X_T654[n, -1 + k0 + x0, -1 + k1 + x1, c]), k0 < 3, k1 < 3, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_T653 shape: fp32(1, 28, 28, 44):(34496, 1232, 44, 1):134.75 KiB
// Elementwise op: [[pid(normal_A_block_0)]] X_T656 = div(X_T653, X_T655)
// Elementwise op: [[pid(Add)]] X_T657 = add(X_T656, X_T656)
...
// lwork = 256, 1, 1
// gwork = 1792, 2, 1
__kernel void kernel_c42_sdk_236(__global float* restrict  X_T657, __global const float* restrict  in1, __global const float* restrict  X_T653)
{
  in1 = (in1 + -1276);
    ...
    if (x0_cond)
    {
      ...
      for (int c_lid = 0; c_lid < 2; c_lid += 1)
      {
        ...
        if (c_cond)
        {
          ...
          float LX_T655 = agg[(c_lid + (x0_lid * 2))];
          int gout_idx = ((c + (1232 * (x0_gid + x0))) + (44 * (x1_gid + x1_tid)));
          if (((gout_idx >= 0) && (gout_idx < 34496)))
          {
            float LX_T653 = X_T653[gout_idx];
            float LX_T656 = (LX_T653 / LX_T655);
            float LX_T657 = (LX_T656 + LX_T656);
            X_T657[gout_idx] = LX_T657;
          }
        }
      }
    }
