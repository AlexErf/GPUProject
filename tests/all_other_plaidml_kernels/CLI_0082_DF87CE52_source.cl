...
// lid: 256 1 1
// Original:
// X_T99[n, x0, x1, c : _T108, _T109, _T110, _T111] = +(X_T98[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 3, k1 < 3
// With Index Variables Made Integral:
// X_T99[n, x0, x1, c : _T108, _T109, _T110, _T111] = +(X_T98[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 3, k1 < 3, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_T96 shape: fp32(1, 56, 56, 11):(34496, 616, 11, 1):134.75 KiB
// Elementwise op: [[pid(reduction_A_block_stem_1)]] X_T100 = div(X_T96, X_T99)
// Tile size: { 11, 3, 3, 8, 8 }
// Contraction output var shape: fp32(1, 56, 56, 11):(34496, 616, 11, 1):134.75 KiB
// Computed true ops: 931392
...
// lwork = 256, 1, 1
// gwork = 1792, 7, 1
__kernel void kernel_c42_sdk_22(__global float* restrict  X_T100, __global const float* restrict  in1, __global const float* restrict  X_T96)
{
  ...
  if (c_cond)
  {
    for (int x1_lid = 0; x1_lid < 2; x1_lid += 1)
    {
      ...
      {
        int x0 = ((4 * x0_lid) + x0_tid);
        float LX_T99 = agg[(x1_lid + (x0_lid * 2))];
        int gout_idx = ((c_tid + (616 * (x0_gid + x0))) + (11 * (x1_gid + x1)));
        float LX_T96 = X_T96[gout_idx];
        float LX_T100 = (LX_T96 / LX_T99);
