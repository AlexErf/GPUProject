...
// lid: 256 1 1
// Original:
// X_T98[x0, y1 : _T134, _T135] = +(X_T97[x0, z] * X_I_1[z, y1])
// With Index Variables Made Integral:
// X_T98[x0, y1 : _T134, _T135] = +(X_T97[x0, z] * X_I_1[z, y1]), 500000000 + x0 < 1000000000, 500000000 + y1 < 1000000000, 500000000 + z < 1000000000
// Constraints:{ 0 <= x0 < 1, 0 <= x0 < 1, 0 <= y1 < 1000, 0 <= y1 < 1000, 0 <= z < 4096, 0 <= z < 4096, 0 <= 500000000 + x0 < 1000000000, 0 <= 500000000 + y1 < 1000000000, 0 <= 500000000 + z < 1000000000 }
// Merged Parallel Constraints:{ 0 <= x0 < 1, 0 <= y1 < 1000, 0 <= z < 4096 }
// Defracted:
// X_T98[x0, y1 : _T134, _T135] = +(X_T97[x0, z] * X_I_1[z, y1]), 500000000 + x0 < 1000000000, 500000000 + y1 < 1000000000, 500000000 + z < 1000000000
// Flattened:
//              Range     X_T98     X_T97     X_I_1  
//       y1      1000         1         0         1  
//        z      4096         0         1      1000  
//      off                   0         0         0  
//      vec                   1         1         1  
...
// Elementwise op: [[pid(Add)]] X_T99 = add(X_T98, X_I_0)
// Tile size: { 64, 64 }
// Contraction output var shape: fp32(1, 1000):(1000, 1):3.90625 KiB
// Computed true ops: 12288000
// Computed work groups: 16
// Computed inner loops: 64
...
// lwork = 256, 1, 1
// gwork = 4096, 1, 1
__kernel void kernel_c18_sdk_20(__global float* restrict  X_T99, __global const float* restrict  in1, __global const float* restrict  in2, __global const float* restrict  X_I_0)
{
  ...
  for (int z_gid = 0; z_gid < 4096; z_gid += 64)
  {
    {
      int z_tid = (tid % 64);
      if ((tid < 64))
    ...
    }
    {
      ...
      for (int z_lid = 0; z_lid < 16; z_lid += 1)
      {
        ...
        in2_shared[lidx] = in2[clamp((int)gidx, (int)0, (int)4095999)];
      }
    }
  ...
  if (y1_cond)
  {
    if ((tid < 64))
    {
      ...
      float LX_T99 = (LX_T98 + LX_I_0);
      X_T99[gout_idx] = LX_T99;
    }
  }
}
