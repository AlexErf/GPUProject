...
// lid: 256 1 1
// Original:
// X_T251[n, x0, x1, c : _T341, _T342, _T343, _T344] = >(X_T250[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 3, k1 < 3
// With Index Variables Made Integral:
// X_T251[n, x0, x1, c : _T341, _T342, _T343, _T344] = >(X_T250[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 3, k1 < 3, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// lwork = 256, 1, 1
// gwork = 2816, 11, 1
__kernel void kernel_c42_sdk_78(__global float* restrict  X_T251, __global const float* restrict  in1)
{
      ...
      if (x0_cond)
      {
        ...
        for (int c_lid = 0; c_lid < 3; c_lid += 1)
        {
          ...
          if (c_cond)
          {
            ...
            float LX_T251 = agg[(c_lid + (x0_lid * 3))];
            LX_T251 = select((float)LX_T251, (float)0, (int)(LX_T251 == (float)-FLT_MAX));
            int gout_idx = ((c + (3528 * (x0_gid + x0))) + (84 * (x1_gid + x1_tid)));
            X_T251[gout_idx] = LX_T251;
          }
        }
      }
    }
