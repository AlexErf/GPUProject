...
// lid: 256 1 1
// Original:
// X_T567[n, x0, x1, c : _T753, _T754, _T755, _T756] = +(X_T566[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 2, k1 < 2
// With Index Variables Made Integral:
// X_T567[n, x0, x1, c : _T753, _T754, _T755, _T756] = +(X_T566[n, k0 + 2*x0, k1 + 2*x1, c]), k0 < 2, k1 < 2, 500000000 + c < 1000000000, 500000000 + k0 < 1000000000, 500000000 + k1 < 1000000000, 500000000 + n < 1000000000, 500000000 + x0 < 1000000000, 500000000 + x1 < 1000000000
...
// Elementwise input X_T565 shape: fp32(1, 14, 14, 256):(50176, 3584, 256, 1):196 KiB
// Elementwise input X_I_212 shape: fp32(256):(1):1 KiB
// Elementwise input X_I_211 shape: fp32(256):(1):1 KiB
// Elementwise op: [[pid(pool3_pool)]] X_T568 = div(X_T565, X_T567)
// Elementwise op: [[pid(Sub)]] X_T574 = sub(X_T568, X_I_212)
// Elementwise op: [[pid(Mul)]] X_T575 = mul(X_T574, X_I_211)
...
// lwork = 256, 1, 1
// gwork = 3584, 4, 1
__kernel void kernel_c108_sdk_179(__global float* restrict  X_T568, __global float* restrict  X_T575, __global const float* restrict  in1, __global const float* restrict  X_T565, __global const float* restrict  X_I_212, __global const float* restrict  X_I_211)
{
  ...
  for (int x0_lid = 0; x0_lid < 2; x0_lid += 1)
  {
    ...
    if (x0_cond)
    {
      ...
      {
        int c = ((32 * c_lid) + c_tid);
        float LX_T567 = agg[(c_lid + (x0_lid * 2))];
        int gout_idx = (((c_gid + c) + (3584 * x0)) + (256 * x1_gid));
        float LX_T565 = X_T565[gout_idx];
        float LX_I_212 = X_I_212[(c_gid + c)];
        float LX_I_211 = X_I_211[(c_gid + c)];
        float LX_T568 = (LX_T565 / LX_T567);
        float LX_T574 = (LX_T568 - LX_I_212);
        float LX_T575 = (LX_T574 * LX_I_211);
        X_T568[gout_idx] = LX_T568;
        X_T575[gout_idx] = LX_T575;
      }
    }
  }
}
